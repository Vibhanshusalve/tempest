  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if option is None:
      ...
      if is_activated:
        ...
        setup_embed.description += "\n✅ | Checking Olympus's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n✅ | Crafting and configuring the Olympus Supreme role..."
        await setup_message.edit(embed=setup_embed)
        ...
        await asyncio.sleep(1)
        setup_embed.description += "\n✅ | Ensuring precise placement of the Olympus Supreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
                        ...
                        if log_channel:
                            embed_log = discord.Embed(
                                ...
                                description=(
                                    ...
                                    f"**• ID**: {user.id}\n\n"
                                    f"**🛡️ Removed By**: **[Olympus#9545](https://discord.com/users/1144179659735572640)**\n"
                                ),
                                color=0x000000
                            )
...
class Global(commands.Cog):
    ...
    @commands.command(name="globalban",help="Bans the user from all guilds where the bot is present.")
    @commands.is_owner()
    async def global_ban(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        # Get all guilds where the bot is present (not just mutual guilds)
        all_guilds = list(self.client.guilds)
        mutual_guilds = [guild for guild in all_guilds if guild.get_member(user.id)]
        mutual_count = len(mutual_guilds)
        total_count = len(all_guilds)

        confirm_embed = discord.Embed(
            title=f"Are you sure to Ban {user.display_name} Globally?",
            description=f"The user is in **{mutual_count}** mutual guilds with the bot.\nThe bot is in **{total_count}** total guilds.\n\n⚠️ **Note:** This will attempt to ban the user from ALL guilds where the bot is present, regardless of role hierarchy.\n\nGlobal Ban Requestor: {ctx.author.mention}",
            color=0x000000
        )
        ...
        async def confirm(interaction):
            ...
            await ctx.send(f"Processing global ban for {user.name} across {total_count} guilds...")
            success, failure, skipped = [], [], []
            
            # Attempt to ban from ALL guilds where the bot is present
            for guild in all_guilds:
                try:
                    # Check if bot has ban permissions
                    if not guild.me.guild_permissions.ban_members:
                        skipped.append(f"{guild.name} (No ban permission)")
                        continue
                    
                    # Check if user is server owner (cannot ban)
                    if guild.owner_id == user.id:
                        skipped.append(f"{guild.name} (Server owner)")
                        continue
                    
                    # Attempt to ban - Discord API will enforce role hierarchy
                    # If bot's role is lower, this will raise Forbidden exception
                    try:
                        await guild.ban(user, reason=f"Global Ban: {reason}", delete_message_days=0)
                        success.append(guild.name)
                    except discord.Forbidden:
                        # Role hierarchy or permission issue
                        failure.append(f"{guild.name} (Role hierarchy/Permission denied)")
                    except discord.HTTPException as e:
                        failure.append(f"{guild.name} (Error: {str(e)})")
                except Exception as e:
                    failure.append(f"{guild.name} (Unexpected error: {str(e)})")
            
            embed = discord.Embed(
                title="Global Ban Complete",
                description=f"Attempted to ban the user from {total_count} guilds.\n\n✅ **Successful:** {len(success)}\n❌ **Failed:** {len(failure)}\n⚠️ **Skipped:** {len(skipped)}",
                color=0x000000
            )
            embed.add_field(name="✅ Successful", value=f"{len(success)} Guilds", inline=True)
            embed.add_field(name="❌ Failed", value=f"{len(failure)} Guilds", inline=True)
            embed.add_field(name="⚠️ Skipped", value=f"{len(skipped)} Guilds", inline=True)
            
            success_button = Button(label="List Successful", style=discord.ButtonStyle.green)
            failure_button = Button(label="List Failed", style=discord.ButtonStyle.red)
            skipped_button = Button(label="List Skipped", style=discord.ButtonStyle.grey) if skipped else None
            new_view = View()
            new_view.add_item(success_button)
            new_view.add_item(failure_button)
            if skipped_button:
                new_view.add_item(skipped_button)
            ...
            async def list_failure(interaction):
                ...
                embeds = DescriptionEmbedPaginator(entries=entries, description="", title=f"Failed Bans [{len(failure)}]", color=0x000000, per_page=10).get_pages()
                paginator = Paginator(ctx, embeds)
                await paginator.paginate()

            async def list_skipped(interaction):
                if interaction.user != ctx.author:
                    return await interaction.response.send_message("This interaction is not for you.", ephemeral=True)
                entries = [f"{i+1}. {name}" for i, name in enumerate(skipped)]
                embeds = DescriptionEmbedPaginator(entries=entries, description="", title=f"Skipped Guilds [{len(skipped)}]", color=0x000000, per_page=10).get_pages()
                paginator = Paginator(ctx, embeds)
                await paginator.paginate()
            ...
            if skipped_button:
                skipped_button.callback = list_skipped

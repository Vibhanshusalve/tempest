...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="Tempest Customrole | Role Added")
        else:
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=role_id)
            await member.remove_roles(role, reason="Tempest Customrole | Role Removed")
        else:
            raise discord.Forbidden("Bot does not have permission to manage roles.")
...
""")
    ...
    @commands.guild_only()
    async def enable(self, ctx):
        Tempest = ['213347081799073793', '677952614390038559']
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"Tempest Uptime", color=self.color)
...
class Extraowner(commands.Cog):
    ...
    async def extraowner(self, ctx, option: str = None, user: discord.Member = None):
        ...
        Tempest = ['1070619070468214824', '677952614390038559']
        if ctx.author.id != ctx.guild.owner_id and str(ctx.author.id) not in Olympus:
            embed = discord.Embed(title="❌ Access Denied",
...
class General(commands.Cog):
  ...
  async def invite(self, ctx: commands.Context):
    embed = discord.Embed(title="Tempest Invite & Support!",
      description=
      f"> • **[Tempest - Invite Bot](https://discord.com/oauth2/authorize?client_id=1144179659735572640&permissions=2113268958&scope=bot)**\n> • **[Tempest - Support](https://discord.gg/odx)**",
      color=0x000000)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (member.guild.id,)) as cursor:
                    ...
                    if after.channel and role not in member.roles:
                        await self.add_role_with_retry(member, role, reason="Member Joined VC | Tempest Invcrole")
                    elif not after.channel and role in member.roles:
                        await self.remove_role_with_retry(member, role, reason="Member Left VC | Tempest Invcrole")
        except discord.Forbidden:
...
class MapView(ui.View):
    ...
    def get_coordinates(self, location):
        ...
        try:
            headers = {'User-Agent': 'Tempest Bot (https://olyumpus.vercel.app)'}
...
class Owner(commands.Cog):
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="• Tempest Staffs", description=f"\n{staff_display}", color=0x000000)
    ...
    async def _slist(self, ctx):
        ...
        embeds = DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of Tempest [{len(self.client.guilds)}]",
            color=0x000000,
            per_page=10).get_pages()
        paginator = Paginator(ctx, embeds)
        await paginator.paginate()
    ...
    async def _restart(self, ctx: Context):
        await ctx.reply("Restarting Tempest...")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        embeds = DescriptionEmbedPaginator(
            entries=entries,
            title=f"Tempest Owners [{len(nplist)}]",
            description="",
            per_page=10,
            color=0x000000).get_pages()
        paginator = Paginator(ctx, embeds)

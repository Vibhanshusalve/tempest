...
class Music(commands.Cog):
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            embed.add_field(name="Source", value=f"[🎵 Listen on Spotify]({track.uri})" if "spotify" in track.source else f"[🎵 Listen on JioSaavn]({track.uri})" if "jiosaavn" in track.source else f"[🎵 Listen on SoundCloud]({track.uri})" if "soundcloud" in track.source else f"[▶️ Listen on YouTube]({track.uri})")
    ...
    async def play_source(self, ctx, query):
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"• Added [{track.title}](https://discord.gg/odx) to the queue.", color=0x000000))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"• Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/odx)** to the queue.", color=0x000000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"• Added all tracks from album **[{album_info['name']}](https://discord.gg/odx)** to the queue.", color=0x000000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
